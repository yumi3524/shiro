...
class Meks_Instagram_Widget extends WP_Widget {
	...
	function generate_transient_key( $usernames_or_hashtags ) {
		...
		$transient_key = md5( 'meks_instagram_widget_' . $id );
	...
	protected function get_instagram_data( $url ) {
		...
		if ( is_wp_error( $request ) || empty( $request ) ) {
			return new WP_Error( 'invalid_response', esc_html__( 'Unable to communicate with Instagram.', $this->widget_text_domain ) );
		}
		...
		if ( empty( $data ) ) {
			return new WP_Error( 'bad_json', esc_html__( 'Instagram has returned empty data. Please check your username/hashtag.',  $this->widget_text_domain ) );
		}
		...
		if ( isset( $data['entry_data']['ProfilePage'][0]['graphql']['user']['edge_owner_to_timeline_media']['edges'] ) ) {
			...
			return new WP_Error( 'bad_json_2', esc_html__( 'Instagram has returned invalid data.',  $this->widget_text_domain ) );
		}
